...
impl Explorer{
    ...
    fn handle_combine_resource_request(&mut self, to_generate: ComplexResourceType)->ExplorerToOrchestrator<Bag>{
        ...
        if let Ok(PlanetToExplorer::CombineResourceResponse { complex_response }) = self.planet_receiver.recv(){
            match complex_response{
                Ok(complex_resource)=>{
                    self.bag.insert_complex_resource_in_bag(complex_resource);
    ...
    fn make_aipartner(&mut self)->Option<ComplexResourceRequest>{
        let r= self.bag.take_complex_resource(ComplexResourceType::Robot)?;
        let d= self.bag.take_complex_resource(ComplexResourceType::Diamond)?;
        let r = r.to_robot().ok()?;
    ...
    fn make_dolphin(&mut self)->Option<ComplexResourceRequest>{
        let w= self.bag.take_complex_resource(ComplexResourceType::Water)?;
        let l= self.bag.take_complex_resource(ComplexResourceType::Life)?;
        let w= w.to_water().ok()?;
        let l = l.to_life().ok()?;
    ...
    fn make_robot(&mut self)->Option<ComplexResourceRequest>{
        let s= self.bag.take_basic_resource(BasicResourceType::Silicon)?;
        let l= self.bag.take_complex_resource(ComplexResourceType::Life)?;
    ...
    fn make_life(&mut self)->Option<ComplexResourceRequest>{
        let w= self.bag.take_complex_resource(ComplexResourceType::Water)?;
        let c= self.bag.take_basic_resource(BasicResourceType::Carbon)?;
        let w= w.to_water().ok()?;
    ...
    fn make_diamond(&mut self)->Option<ComplexResourceRequest>{
        let c1= self.bag.take_basic_resource(BasicResourceType::Carbon)?;
        let c2= self.bag.take_basic_resource(BasicResourceType::Carbon)?;
    ...
    fn make_water(&mut self)->Option<ComplexResourceRequest>{
        let h= self.bag.take_basic_resource(BasicResourceType::Hydrogen)?;
        let o= self.bag.take_basic_resource(BasicResourceType::Oxygen)?;
    ...
    fn handle_generate_resource_request(&mut self, to_generate: BasicResourceType)->ExplorerToOrchestrator<Bag>{
        ...
        if let Ok(PlanetToExplorer::GenerateResourceResponse { resource }) = self.planet_receiver.recv(){
            match resource{
                Some(r) => { 
                    self.bag.insert_basic_resource_in_bag(r);
    ...
    fn handle_bag_content_request(&self)->ExplorerToOrchestrator<Bag>{
        ExplorerToOrchestrator::BagContentResponse { explorer_id: self.id, bag_content: self.bag.clone() }
    }
